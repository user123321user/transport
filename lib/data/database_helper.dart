...
// دالة لحساب الرصيد الإجمالي الحالي المتوفر في الصندوق (الوارد - الصادر)
  ...
  // ---- جلب حركات الصندوق مصفاة بنطاق تاريخ ----
  Future<List<Map<String, dynamic>>> getCashboxByDateRange(String startDate, String endDate) async {
    final db = await DatabaseHelper.instance.database;
    return await db.query('cashbox', where: 'date >= ? AND date <= ?', whereArgs: [startDate, endDate], orderBy: 'transaction_id DESC');
  }

  // ---- جلب رحلات السائقين مصفاة بنطاق تاريخ ----
  Future<List<Map<String, dynamic>>> getDriverTripsByDateRange(String driverName, String startDate, String endDate) async {
    final db = await DatabaseHelper.instance.database;
    return await db.query(
        'trips',
        where: 'driver_name = ? AND date >= ? AND date <= ?',
        whereArgs: [driverName, startDate, endDate],
        orderBy: 'trip_id DESC'
    );
  }
