...
class _CashboxScreenState extends State<CashboxScreen> {
  ...
  void _saveTransaction() async {
    ...
    await dbHelper.insertTransactionMutiCurrency(
      _detailsController.text.trim(), amount, _selectedCurrency, _transactionType,
    );

    _detailsController.clear(); _amountController.clear(); _loadDualBalances();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تسجيل الحركة في حساب العملة المحدد بنجاح')));
  }
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(child: Card(color: Colors.green.shade700, child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [const Text('حساب الليرة السورية', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text('${_balanceSyp.toStringAsFixed(0)} ل.س', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))])))),
                  const SizedBox(width: 10),
                  Expanded(child: Card(color: Colors.indigo.shade700, child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [const Text('حساب الدولار الصافي', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text('\$ ${_balanceUsd.toStringAsFixed(1)}', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))])))),
                ],
              ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 10),
                      Row(children: [Expanded(child: TextField(controller: _amountController, decoration: const InputDecoration(labelText: 'المبلغ المالي', border: OutlineInputBorder()), keyboardType: TextInputType.number)), const SizedBox(width: 10), Expanded(child: DropdownButtonFormField<String>(value: _selectedCurrency, decoration: const InputDecoration(border: OutlineInputBorder()), items: const [DropdownMenuItem(value: 'SYP', child: Text('ليرة سورية (SYP)')), DropdownMenuItem(value: 'USD', child: Text('دولار أمريكي (USD)'))], onChanged: (val) => setState(() => _selectedCurrency = val!)))]),
                      Row(mainAxisAlignment: MainAxisAlignment.center, children: [Radio<String>(value: 'income', groupValue: _transactionType, activeColor: const Color(0xFF1A237E), onChanged: (v) => setState(() => _transactionType = v!)), const Text('وارد (قبض)'), const SizedBox(width: 24), Radio<String>(value: 'expense', groupValue: _transactionType, activeColor: const Color(0xFF1A237E), onChanged: (v) => setState(() => _transactionType = v!)), const Text('صادر (مصروف)')]),
                      ElevatedButton.icon(onPressed: _saveTransaction, icon: const Icon(Icons.check_circle), label: const Text('تثبيت الحركة بالصندوق العملة المحدد'), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1A237E), foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 48)))
                    ],
                  ),
                ),
              ),
