...
class _CashboxScreenState extends State<CashboxScreen> {
  ...
  void _showEditTransactionDialog(int id, String currentDetails, double income, double expense) {
    final detailsController = TextEditingController(text: currentDetails);
    final amountController = TextEditingController(text: income > 0 ? income.toString() : expense.toString());
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('تعديل القيد المالي'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: detailsController, decoration: const InputDecoration(labelText: 'البيان')),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await dbHelper.updateTransaction(id, detailsController.text.trim(), income > 0 ? amt : 0.0, expense > 0 ? amt : 0.0);
              Navigator.pop(ctx);
              _loadBalance();
              setState(() {});
            },
            child: const Text('تعديل القيد'),
          )
        ],
      ),
    );
                            ...
                            trailing: Row(
                              ...
                              children: [
                                ...
                                IconButton(
                                  ...
                                  onPressed: () => _showEditTransactionDialog(item['transaction_id'], item['details'], item['income'], item['expense']),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.delete, color: Colors.red),
                                  onPressed: () async {
                                    await dbHelper.deleteTransaction(item['transaction_id']);
                                    _loadBalance();
                                    setState(() {});
                                  },
                                ),
