...
class _PdfReportScreenState extends State<PdfReportScreen> {
  ...
  Future<void> _generateAndPrintPdf() async {
    ...
    if (_reportType == 'trips' || _reportType == 'drivers') {
      headers = ['رقم الحركة', 'الشاحنة', 'السائق', 'المادة', 'الكمية', 'التاريخ', 'أجرة السائق', 'المصاريف'];
    } else if (_reportType == 'cashbox') {
    ...
    for (var row in data) {
      ...
      if (_reportType == 'trips' || _reportType == 'drivers') {
        rowCells = [
          ...
          row['date'].toString(),
          row['driver_wage']?.toString() ?? '0',
          row['discount_admin']?.toString() ?? '0' // إعادة حقن المصاريف في خلايا جدول الـ PDF
        ];
      } else if (_reportType == 'cashbox') {
        rowCells = [
