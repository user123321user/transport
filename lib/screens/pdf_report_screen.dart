...
import 'package:printing/printing.dart'; // حزمة جلب خطوط جوجل السحابية المدمجة آلياً
  ...
  Future<void> _generateAndPrintPdf() async {
    if (_selectedDateRange == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('الرجاء تحديد نطاق التاريخ أولاً')),
      );
      return;
    }
    ...
    // 1. جلب بايتات خط Cairo العربي الأصلي مباشرة من السيرفر بشكل مشفر وسليم 100%
    final pw.Font arabicFont = await PdfGoogleFonts.cairoRegular();
    final pw.Font arabicFontBold = await PdfGoogleFonts.cairoBold();
    ...
    // 2. بناء صفوف الجدول يدوياً لضمان حقن وتمرير الخط العربي لكل نص بداخل كل خلية
    List<pw.TableRow> tableRows = [];

    // إضافة العناوين الرئيسية للجدول (Header)
    List<String> headers = [];
        ...
        children: headers.map((headerText) {
          return pw.Padding(
            ...
            child: pw.Center(
              child: pw.Text(
                headerText,
                style: pw.TextStyle(font: arabicFontBold, fontSize: 11, fontWeight: pw.FontWeight.bold),
              ),
            ),
          );
        }).toList(),
    ...
    // إضافة بيانات الحسابات والرحلات (Rows) وتغليف كل نص بالخط العربي قسرياً لمنع الـ XXXX
    for (var row in data) {
          ...
          children: rowCells.map((cellText) {
            return pw.Padding(
              ...
              child: pw.Container(
                ...
                child: pw.Text(
                  cellText,
                  style: pw.TextStyle(font: arabicFont, fontSize: 10), // هنا التمرير الإجباري لكل خلية
                ),
              ),
            );
          }).toList(),
    ...
    // 3. بناء الصفحة وتفعيل معايير الـ RTL (من اليمين لليسار) للجدول والعناوين
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        build: (pw.Context context) {
          return pw.Directionality(
            textDirection: pw.TextDirection.rtl, // التوجيه التلقائي للعربية
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Center(child: pw.Text(title, style: pw.TextStyle(font: arabicFontBold, fontSize: 20, fontWeight: pw.FontWeight.bold))),
                ...
                // عرض الجدول المطور الخالي تماماً من المربعات والرموز
                pw.Table(
    ...
    // 4. فتح واجهة نظام التشغيل الأصلية للطباعة أو التصدير
