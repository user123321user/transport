...
class _SystemSetupScreenState extends State<SystemSetupScreen> with SingleTickerProviderStateMixin {
  ...
  void _showEditAccountDialog(int id, String currentName, String currentType, int isPermanent, double currentBalance) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: const Text('تعديل حقول الحساب / المستخدم'),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                TextField(controller: balanceController, decoration: const InputDecoration(labelText: 'الرصيد المالي'), keyboardType: TextInputType.number),
                CheckboxListTile(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                double bal = double.tryParse(balanceController.text) ?? 0.0;
                await dbHelper.updateAccount(id, nameController.text.trim(), selectedType, permanent ? 1 : 0, bal);
                Navigator.pop(ctx);
                setState(() {});
              },
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text('التصنيف: ${item['type']} | الرصيد: ${item['balance']}'),
                            trailing: Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                IconButton(
                                  icon: const Icon(Icons.edit, color: Colors.blue),
                                  onPressed: () => _showEditAccountDialog(accId, item['name'], item['type'], item['is_permanent'] ?? 0, item['balance'] ?? 0.0),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.delete, color: Colors.red),
                                  onPressed: () async {
